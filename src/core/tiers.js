...
export function getTierForUser(userId, db) {
  ...
  return db.get('SELECT * FROM account_tiers WHERE name = ?', tierName) || { name: 'free', max_links_per_hour: 10, max_file_size_mb: 10, allow_raw_html: 1, show_ads: 1, max_links_total: 0, max_images_total: 0, max_text_total: 0, allowed_image_types: 'image/jpeg,image/png,image/gif' }
}

export function getAnonymousTier(db) {
  return db.get("SELECT * FROM account_tiers WHERE name = 'anonymous'") ||
    { name: 'anonymous', max_links_per_hour: 5, max_links_total: 10, max_images_total: 3, max_text_total: 5, max_file_size_mb: 5, allow_raw_html: 0, show_ads: 1, allowed_image_types: 'image/jpeg,image/png,image/gif' }
}
